...
export function DashboardStats() {
    ...
    const { isViewer, isSuperAdmin, isAdmin } = usePermissions();
    ...
    // Actualizar fecha cada segundo (para todos los usuarios)
    useEffect(() => {
    ...
    // Seleccionar frase del día aleatoria (para todos los usuarios)
    useEffect(() => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Información adicional (fecha, hora, clima, frase) */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {infoCards.map((card) => {
                    const Icon = card.icon;
                    const isQuote = 'isQuote' in card && card.isQuote;
                    ...
                    return (
                        <Card key={card.title} className="bg-gradient-to-br from-background to-muted/20">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">
                                    {card.title}
                                </CardTitle>
